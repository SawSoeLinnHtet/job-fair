<?php
  include("../../../vendor/autoload.php");

  use Helpers\URI;
?>

<section class="sidebar">
  <ul class="sidebar-items">
    <li>
      <a href="../dashboard/" id="<?= URI::check("/dashboard/"); ?>">
        <span class="icon icon-1">
          <i class="ri-layout-2-line"></i>
        </span>
        <span class="sidebar-item" style="white-space: nowrap;">DASHBOARD</span>
      </a>
    </li>
    <li>
      <a href="../users/" id="<?= URI::check("/users/"); ?>">
        <span class="icon icon-2">
          <i class="ri-user-fill"></i>
        </span>
        <span class="sidebar-item">USERS</span>
      </a>
    </li>
    <li>
      <a href="../jobs/" id="<?= URI::check("/jobs/"); ?>">
        <span class="icon icon-4">
          <i class="ri-briefcase-2-fill"></i>
        </span>
        <span class="sidebar-item">JOBS</span>
      </a>
    </li>
    <li>
      <a href="../companies/" id="<?= URI::check("/companies/"); ?>">
        <span class="icon icon-5">
          <i class="ri-community-line"></i>
        </span>
        <span class="sidebar-item">COMPANIES</span>
      </a>
    </li>
    <li>
      <a href="../categories/" id="<?= URI::check("/categories/"); ?>">
        <span class="icon icon-5">
          <i class="ri-grid-fill"></i>
        </span>
        <span class="sidebar-item">CATEGORIES</span>
      </a>
    </li>
    <li>
      <a href="../applicatiants/companies.php" id="<?= URI::check("/applicatiants/"); ?>">
        <span class="icon icon-5">
          <i class="ri-list-check-2"></i>
        </span>
        <span class="sidebar-item">APPLICATIANTS</span>
      </a>
    </li>
  </ul>
  <ul class="bottom-sidebar-items">
    <li>
      <a href="#">
        <span class="icon icon-6">
          <i class="ri-settings-2-fill"></i>
        </span>
        <span class="sidebar-item">SETTING</span>
      </a>
    </li>
    <li>
      <a href="../../../App/controllers/logout.php">
        <span class="icon icon-7">
          <i class="ri-logout-box-r-line"></i>
        </span>
        <span class="sidebar-item">LOGOUT</span>
      </a>
    </li>
  </ul>
</section>